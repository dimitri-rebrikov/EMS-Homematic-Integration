...
- put thermostats into a passive mode if the boiler's heating function is disabled
...
The use cases logic is implemented on the CCU2 using the Homematic scripting language. 
The scripts on the CCU2 communicate with the Thermostats using the CCU Object Model and with the boiler using the EMS-ESP REST API.
...
It was originally closed source standard intended for internal communication between the hardware of the manufacturer but it was reverse engineered by the community and now is available both as description but also as several open source projects/libraries.
...
Additionally there is also a REST API to access the settings by any other applications connected to the same network.

## Hardware
There are ready to use hardware modules sold by https://bbqkees-electronics.nl/ .
I decided to build my own hardware module to have a better understanding how it works.

I found the basic circuit schematic in the old version of the now archived [EMSESP8266 project](https://github.com/dimitri-rebrikov/EMS-ESP/blob/1.9.4/doc/schematics/Schematic_EMS-ESP.png). The schematic is outdated and was later removed from the sources. The reason is probably to concentrate on the software and also provide more reason to buy the ready-to-use hardware modules from bbqkees.

I adapted the circuit to be supplied by the power from the VCC+ pin of the EMS power jack of the boiler.
And I used ESP32 instead of ESP8266 as the current EMS-ESP firmware version does not support the ESP8266 anymore.
...
Each module is a separate circuit board of nearly the same size.
They assembled together as a stack using the brass distance sleeves respective thick soldering wires. 
I put the assembly into a standard electric distribution box:
...
5. copy the user access token from the security settings into the corresponding CCU system variables. The access token is necessary for the write operations to the boiler
...
In order to work with the project the thermostats needs to be paired with the CCU. For the paring and other details  google for the word "manual" and the type name of the corresponding thermostat.
After the paring the status values of the thermostat can be viewed/changed over the CCU either over the user interface or by a script accessing the CCU object model.

# Use Cases implementation
The user cases logic is implemented in several scripts.
All scripts have the following in common: They
- run on the CCU
- implemented using the Homematic scripting language
...
- the CCU script [Prog.ReadThermostatStatus.txt](./Prog.ReadThermostatStatus.txt) implements the detection of the desired temperature and stores it
...
## Use Case "Send boost mode to the boiler"
This is a convenience feature implemented both in the Homematic thermostats and in the Bosch boiler controller.
In the thermostats it results in the full opening of the valve for a configured amount of time. In the boiler controller it results in setting the boiler to the configured "boost" temperature (f.e. 30 degree) for a configured amount of time. But without synchronization between thermostats and boiler this feature is pretty useless. So opening the valve on the thermostat will not result the boiler switching into the higher power mode. The boiler will still try to keep the configured temperature by controlling the return water temperature and cancelling heating if the water becomes too hot. And vice versa: switching the boiler into the boost mode will use nothing if the valve of the thermostat is closed as it configured to keep the normal room temperature. 

The use case implements the following workflow:
- iterate over all thermostats every 10 sec
  - check if any of the thermostat is put into the boost mode 
  - store the result of the check in a system variable
- depending on the stored result enable/disable the boiler's boost mode every 10 sec

The CCU elements used in the use case:
- the CCU function `ThermostatList` stores the list of the thermostats to iterate
- the CCU script [Prog.ReadThermostatStatus.txt](./Prog.ReadThermostatStatus.txt) implements the check for the thermostats boost mode and stores it
- the CCU system variable `SysVar.ThermostatBoostMode` contains the last result of the thermostats boost mode check
...
## Use Case "Send the boilers heating disabled status to the thermostats"
The boiler is equipped with an outdoor temperature sensor. Based on the configured outdoor temperature threshold the boiler enables/disables the heating function. If the heating function of the boiler is disabled there is no reason for the thermostats to provide any kind of reaction to the temperature. They can be put into the "valve is always on" mode preventing battery drain from coming the valve motor actions but also preventing the valves from corrosion.
